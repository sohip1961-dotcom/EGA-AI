...
>
> **Last updated:** 2026-09-02 (Conversational Fast-Lane & Student Retention Engine: Implemented sub-second greeting fast-lane bypassing heavy RAG, calibrated out-of-curriculum warning to exclude pleasantries, active first-turn [QUIZ_QUESTION] hook with gamified +2 merit points rewards via /api/user/quiz-points, dynamic subject-specific prompt sparks, free coin reassurance badge, and automated 24h student re-engagement lifecycle email engine via /api/cron/study-reminder).
>
...
9. Mode-specific behavioral block (`MODE_INSTRUCTIONS`): **socratic** (scaffolded guiding questions without immediate final answers), **detailed** (structured student-friendly explanations with core formula/takeaway in the first 2 lines, avoiding monolithic walls of text), **summary** (high-yield exam review bullet points).
